...
## 0. ABSOLUTE MANDATE: NO AUTONOMOUS GIT

**NEVER perform any git actions (add, commit, push, stash, etc.) yourself.**

This is an absolute rule. Even if you have finished a task, even if you are in "autonomous mode", even if you think it's the right next step: **YOU MUST STOP AND ASK FOR PERMISSION BEFORE EVERY GIT COMMAND.**

You are allowed to use `git status` or `git diff` for research, but any command that modifies the git state is strictly forbidden unless the user explicitly gives you a directive to execute that specific command.
...
4. **Testing is Mandatory:** Every change needs a corresponding spec. **DO NOT create temporary test files.** When
...
5. **90% Coverage Requirement:** All files in the `src/` directory MUST have a minimum of **90% code coverage**. A task
   is NOT considered finished until this threshold is met for all modified or new files.
6. **Verification:** Before claiming a task is done, run the following commands and ensure they all pass with zero errors
   or warnings:
...
7. **Documentation Maintenance:** For every new feature or syntax addition to the documentation, you MUST add a condensed summary of the syntax in `docs/syntax-reference.zlt` to keep the quick reference guide up to date.
8. **No Placeholders:** Write complete code. Do not leave `// TODO` or `...` inside code blocks unless specifically
   requested by the user.
9. **No any Bypass:** Do not use `any` to bypass type-checking or linting rules. Use proper types, interfaces, or type
   guards.
