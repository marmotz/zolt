...
&&[File]({$f})
    `;
    const html = await buildString(zolt);
    expect(html).toContain('<video src="video.mp4">Video</video>');
    expect(html).toContain('<audio src="audio.mp3">Audio</audio>');
    expect(html).toContain('<iframe src="https://youtube.com/123" title="Embed"></iframe>');
    expect(html).toContain('<a href="doc.pdf">File</a>');
  });
...
:::
    `;
    const html = await buildString(zolt);
    expect(html).toContain('<h1>Tech</h1>');
